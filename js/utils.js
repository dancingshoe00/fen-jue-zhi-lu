...
function respecBuyables(layer) {
	...
	if (!player[layer].noRespecConfirm && !confirm(tmp[layer].buyables.respecMessage || "确定要重置加点吗？这也会触发一次“" + (tmp[layer].name ? tmp[layer].name : layer) + "”重置。")) return
...
function doPopup(type = "none", text = "这是一条测试提示。", title = "", timer = 3, color = "") {
	switch (type) {
		case "achievement":
			popupTitle = "成就达成";
