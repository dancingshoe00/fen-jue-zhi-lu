...
function gameLoop(diff, automaticActions = true) {
...
var interval = setInterval(function() {
	...
	if (player.offTime !== undefined) {
		...
		if (player.offTime.remain > 0) {
			offlineDiff = Math.max(player.offTime.remain / 10, diff)
	...
	updateTemp();
	updateOomps(diff + offlineDiff);
	updateWidth()
	updateTabFormats()
	gameLoop(diff)
	if (offlineDiff > 0) gameLoop(offlineDiff, false)
	fixNaNs()
